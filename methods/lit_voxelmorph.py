...
class LitVoxelMorph(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        rows = list()
        for moving_img, fixed_img, warped_moving_img, warped_fixed_img, forward_dvf_img, backward_dvf_img in zip(moving_image,
                                                                                                                    fixed_image,
                                                                                                                    warped_moving_image,
                                                                                                                    warped_fixed_image,
                                                                                                                    forward_dvf_image,
                                                                                                                    backward_dvf_image):
            displayed_images = normalize(warped_fixed_img[0:1], moving_img[0:1])[::-1] + normalize(warped_moving_img[0:1], fixed_img[0:1])[::-1] + [
    ...
    def test_step(self, batch, batch_idx: int):
        ...
        for save_dir, fused_image in zip([gt_lr_save_dir, pd_hr_save_dir, gt_hr_save_dir], [gt_lr_image, pd_hr_image, gt_hr_image]):
