...
from methods.networks.voxelmorph import utils
...
def semisupervised(vol_names, seg_names, labels, atlas_file=None, downsize=2):
    ...
    if atlas_file:
        trg_vol = utils.load_volfile(atlas_file, np_var='vol',
                                        add_batch_axis=True, add_feat_axis=True)
        trg_seg = utils.load_volfile(atlas_file, np_var='seg',
                                        add_batch_axis=True, add_feat_axis=True)
...
def surf_semisupervised(
    ...
    std_to_surf = lambda x, y: utils.sdt_to_surface_pts(
        x, y,
    ...
    while True:
        ...
        if nb_labels_sample != len(labels):
            ...
            nb_surface_pts_sel = utils.get_surface_pts_per_label(
        ...
        for li, sli in enumerate(sel_label_idxs):
            ...
            X_sdt_k[..., li] = utils.vol_to_sdt_batch(
